...
def process_file(full_file_name, lan, rect_color, corner, label, label_corner, k):
    ...
    if extension == 'tif' or extension == "TIF" or extension == 'tiff':
        try:
            ...
            result = draw_bar(img_cropped, tif_tags, lan, rect_color, corner, label, label_corner)
        ...
        try:
            ...
            result = draw_bar(img_cropped, chunks, lan, rect_color, corner, label, label_corner)
            result.save(f"{folder}/{short_file_name}_cut_{k}.{extension}")
        except:
            print("Error during procession ", full_file_name, ".")
        ...
        if folder is not None:
            for root, dirs, files in os.walk(folder):
                for i, file in enumerate(files):
                    ...
                    process_file(full_file_name, language, rect_color, corner, label, label_corner, k)
            k +=1
            folder = None
