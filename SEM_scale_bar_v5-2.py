...
def get_scale(tags):
    ...
    if 'CZ_SEM' in tags:
        ...
        try:
            pixel_size = float(tags['FEI_HELIOS']['Scan']['PixelWidth'])*1000000 # microns per pixel
        except:
...
def cut_panel(img, tags):
    ...
    if 'CZ_SEM' in tags:
    ...
    elif 'FEI_HELIOS' in tags:      # for FEI, ThermoSci and SM-32 images
        try:
            short_height = tags['FEI_HELIOS']['Scan']['ResolutionY'] 
        except:
            short_height = tags['FEI_HELIOS']['Image']['ResolutionY']
        ...
        try:
            ...
            print("Unknown metadata format. You can send this file to the developer.")
...
def tif2np(tif):
    ...
    if img.dtype != np.uint8:
        img_fin = (img * 255).astype(np.uint8)
    return img_fin
...
def draw_bar(img, tags, lang, rect_color, corner, label, label_corner):
        ...
        img2.line([(abs(width - bar/2 - (rect_width)/2), height - round(30*n)),
                (abs(width - (rect_width - bar)/2), height - round(30*n))],
                fill = bar_color,
                width = round(20*n)
        )
...
def tif_to_L(image):
    ...
    if image.mode in ('I', 'I;16', 'I;16B', 'I;16L'):
        try:
            ...
            image = image.point(lambda x: x / 256).convert("L")  # 0-65535 -> 0-255
...
# Create the window:
window = sg.Window('SEM scale bar - version 5.2', layout)
...
while True:
    ...
    if event == 'Choose folder with SEM images':
        ...
        if first is None:
            ...
            original_img = tif_to_L(original_img) # for ThermoSci images
    ...
    if event == 'Process':
        if folder is not None:
            ...
            #return contrast and brightness to the starting values:
            factor = 1
            contrast = 1
