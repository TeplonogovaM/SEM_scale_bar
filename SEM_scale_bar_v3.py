...
def process_folder(folder, lan, rect_color, corner, k):
    for root, dirs, files in os.walk(folder):
        for i, full_file_name in enumerate(files):
            short_file_name, extension = full_file_name.split('.')
            if extension == 'tif' or extension == "TIF":
                try:
                    tif = tifffile.TiffFile(folder + '/' + full_file_name)
                    ...
                    result.save(f"{folder}/{short_file_name}_{k}.{extension}")
                except:
                    pass
            else:
...
# the same function for one image
...
# Create the window:
window = sg.Window('SEM scale bar - version 3', layout)
...
k = 2 # index for processed images
folder = None
file = None
...
while True:
    ...
    if event == 'Process':
        if folder is not None:
            process_folder(folder, language, rect_color, corner, k)
            k +=1
            folder = None
